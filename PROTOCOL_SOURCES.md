...
Last reviewed against the linked public sources: 2026-08-08.
...
| Egress-client service-token enrollment | Public Cloudflare MDM and Access authentication contract, plus clean-room behavioral validation | Cloudflare documents the One Client MDM [`organization`, `auth_client_id`, and `auth_client_secret` parameters](https://developers.cloudflare.com/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/mdm-deployment/parameters/#auth_client_id), the Device Enrollment [`Service Auth` policy action](https://developers.cloudflare.com/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/device-enrollment/), and the standard Access [`CF-Access-Client-Id` and `CF-Access-Client-Secret` headers](https://developers.cloudflare.com/cloudflare-one/access-controls/service-credentials/service-tokens/). Authorized observation of the official Linux package established that these inputs request an enrollment JWT from the organization Access `/warp` endpoint. This port origin-confines the headers, disables redirects, validates the custom-scheme callback, and sends only the returned JWT to the inherited registration request. |
| Service-token live validation | Authorized interoperability test | A 2026-08-08 test first confirmed fail-closed rejection of mismatched credentials, then validated the complete flow after the dedicated token was rotated through Cloudflare's documented API: Access JWT issuance, non-interactive device registration, P-256 enrollment, truthful device-state authorization, CONNECT-IP with PMTUD, assigned IPv4/IPv6 addresses, zero-loss dual-stack ICMP, and HTTPS egress. Service-token credentials were not logged or persisted in the generated tunnel config. |
...
| CONNECT-IP tunnel lifetime | Public standards and quiche API | RFC 9484 ties the IP tunnel lifetime to its request stream and permits the proxy to close that stream when tearing down an idle tunnel. The runtime retains the original request stream ID and consumes quiche's native HTTP/3 `Finished` and `Reset` events. Termination of that stream ends the session and reaches the existing reconnect supervisor; unrelated H3 request streams do not affect tunnel lifetime. |
...
| Mesh session activation | Observed edge condition implemented only with public standards | Authorized A/B tests on the maintainer's account showed that registration, device-state, `/h3-stats` and idle CONNECT-IP leave the Connector connections API empty; the first inner IP packet creates the connection, after which QUIC keepalive preserves it. Reconnect requires another inner packet. Mesh mode therefore sends exactly one ICMP Echo Request or ICMPv6 Echo Request through the existing RFC 9484 HTTP/3 DATAGRAM path after each successful CONNECT-IP response, defaulting to Cloudflare's 1.1.1.1 service with Mesh-only CLI/config overrides. It adds no endpoint, proprietary field, route, status override or official-client claim, and remains disabled in client mode. |
